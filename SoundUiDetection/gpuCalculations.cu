...
__global__ static void crossCorrelateKernel(const float* noise, const float* signal, float* correlation, int noiseSize, int signalSize) {
...
std::vector<float> correlationGpuOther(std::vector<float>& signal, std::vector<float>& noise) {
    ...
    cudaMalloc((void**)&d_signal, signal.size() * sizeof(float));
    cudaMalloc((void**)&d_noise, noise.size() * sizeof(float));
    cudaMalloc((void**)&d_correlation, (signal.size() - noise.size() + 1) * sizeof(float));

    cudaMemcpy(d_signal, signal.data(), signal.size() * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_noise, noise.data(), noise.size() * sizeof(float), cudaMemcpyHostToDevice);

    int blockSize = 256;
    int numBlocks = (signal.size() - noise.size() + 1 + 1 + blockSize - 1) / blockSize;

    crossCorrelateKernel<<<numBlocks, blockSize>>>(d_noise, d_signal, d_correlation, signal.size(), noise.size());

    std::vector<float> correlation(signal.size() - noise.size() + 1 + 1);
    cudaMemcpy(correlation.data(), d_correlation, correlation.size() * sizeof(float), cudaMemcpyDeviceToHost);
